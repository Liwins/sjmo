package cn.riversky.yanmo.structtype.decorator.e1.db;
...
public class Prize {
    ...
    public  double calcPrize(String user,Date begin,Date end){
        double prize=0.0;
        prize=monthPrize(user,begin,end)+sumPrize(user,begin,end);
        if(isManager(user)){
    ...
    private double monthPrize(String user, Date begin, Date end) {
        ...
        double prize=0.0;
        prize=SaleMoneyDao.mapMonthSaleMoney.get(user)*0.03;
    ...
    public double groupPrize(String user, Date begin, Date end) {
        double group=0.0;
        for(double d:SaleMoneyDao.mapMonthSaleMoney.values()){
            group+=d;
        }
